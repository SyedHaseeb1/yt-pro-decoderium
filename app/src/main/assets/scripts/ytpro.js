// Comprehensive Ad Blocker for YouTube

(() => {
  // 1. Block ad URLs via fetch
              ...
              if(obj[key] && typeof obj[key] === 'object') {
                if(Array.isArray(obj[key])) {
                  obj[key] = obj[key].filter(item => {
                    if(item && item.adSlotRenderer) return false;
                    if(item && item.adBreakRenderer) return false;
                    return true;
                  });
                  obj[key].forEach(removeAds);
                } else {
                  removeAds(obj[key]);
                }
              }
  ...
  // 2. Also intercept XMLHttpRequest
  ...
  XMLHttpRequest.prototype.open = function(method, url, ...args) {
    ...
    return _origOpen.apply(this, [method, url, ...args]);
  };

  // 3. DOM mutation observer to remove ad elements
        ...
        mutation.addedNodes.forEach((node) => {
          if(node.nodeType === 1) { // Element node
            ...
            if(node.classList && (
              ...
              node.classList.contains('ad-div') ||
              node.id === 'masthead-ad' ||
              node.id === 'movie_player' && node.querySelector('[data-ad-break-params]')
            )) {
              node.remove();
            }
            
            // Remove ad attributes
            if(node.hasAttribute && node.hasAttribute('data-ad-break-params')) {
              node.remove();
            }
          }
        });
  ...
  observer.observe(document.body, {
    ...
    subtree: true,
    attributes: true,
    attributeFilter: ['data-ad-break-params', 'data-ads']
  });

  console.log("[AdBlock] Comprehensive ad blocker activated");
})();
...
//ERUDA
var script = document.createElement('script'); script.src="file:///android_asset/scripts/eruda"; document.body.appendChild(script); script.onload=()=>{eruda.init();}
}
/**/
...
margin-left:10px;
`);
refreshDiv.setAttribute("id","refreshBtn");
refreshDiv.innerHTML='↻';
console.log('Creating refresh button');
insertAfter(setDiv,refreshDiv);
console.log('Refresh button inserted');
...
try {
var homeLogoBtn = document.getElementsByTagName("ytm-home-logo")[0];
if(homeLogoBtn) {
homeLogoBtn.addEventListener("click", function(ev) {
console.log('Home logo clicked - clearing back history');
window.location.href = 'https://m.youtube.com';
});
}
} catch(e) {
console.log('Error setting up home logo click:', e);
}
}
...
script2.src="file:///android_asset/scripts/ytpro";
...
script.src="file:///android_asset/scripts/eruda"; 
...
sd.src="file:///android_asset/scripts/showdown/dist/showdown.min.js";
